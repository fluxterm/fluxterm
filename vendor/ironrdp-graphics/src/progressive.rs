...
mod tests {
    ...
    /// 同一瓦片内的离散单行更新必须全部输出，且不能污染未更新像素。
    #[test]
    fn disjoint_thin_updates_preserve_decoded_tile_coverage() {
        use ironrdp_pdu::codecs::rfx::progressive::{
            ProgressiveBlock, decode_progressive_stream, encode_progressive_stream,
        };
        let original = simple_tile_stream(0, [64, -16, 24], true);
        let mut blocks = decode_progressive_stream(&original).unwrap();
        for block in &mut blocks {
            if let ProgressiveBlock::Region(region) = block {
                region.rects = vec![rect(0, 0, 16, 1), rect(32, 0, 16, 1), rect(15, 0, 1, 64)];
            }
        }
        let stream = encode_progressive_stream(&blocks).unwrap();
        let decoded = ProgressiveDecoder::new().decode_bitmap(1, 7, 64, 64, &stream).unwrap();
        assert_eq!(decoded.len(), 1);
        let reference = ProgressiveDecoder::new()
            .decode_bitmap(1, 7, 64, 64, &original)
            .unwrap();
        assert_eq!(decoded[0].pixels, reference[0].pixels);
        for y in 0..64 {
            for x in 0..64 {
                let expected = (y == 0 && (x < 16 || (32..48).contains(&x))) || x == 15;
                let count = decoded[0]
                    .update_rectangles
                    .iter()
