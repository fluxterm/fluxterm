...
impl<'a> SrlDecoder<'a> {
    /// 按 PDU 指定长度读取 SRL；允许省略尾部填充字节，不能提前剥掉有效零字节。
    ...
    pub fn decode(&mut self, num_values: usize, num_bits: u8) -> Result<Vec<i16>, SrlError> {
        ...
        while output.len() < num_values {
            ...
            // 零块逐段消费；已经满足目标系数数目时，不读取不存在的后续码字。
            // 剩余零和 KP 跨子带保留，与逐系数解码顺序一致。
            let k = self.kp / 8;
...
mod tests {
    ...
    /// 合法有效位不需要额外的零字节；最后一个字节也可能全为零。
    #[test]
    fn accepts_length_delimited_srl_without_sentinel() {
        assert_eq!(decode_srl(&[0x84], 1, 4), Ok(vec![3]));
        assert_eq!(decode_srl(&[0xa0, 0x00], 1, 3), Ok(vec![-7]));
    }

    /// 最后一个零游程已经覆盖所需系数时，不再要求额外的结束码字。
    #[test]
    fn final_zero_chunk_can_end_at_component_boundary() {
        assert_eq!(decode_srl(&[0], 60, 1), Ok(vec![0; 60]));
    }
    ...
    fn rejects_truncated_stream() {
        assert_eq!(decode_srl(&[0x80], 1, 4), Err(SrlError::Truncated));
    }

    #[test]
    fn rejects_missing_required_code_bits() {
