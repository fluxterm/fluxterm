...
pub struct GraphicsPipelineClient {
    ...
    /// 尚未被会话画面消费的输出尺寸重置。
    pending_output_reset: Option<(u32, u32)>,
...
impl GraphicsPipelineClient {
    ...
    /// 先消费尺寸重置，再应用同一批次的画面增量。
    pub fn take_output_reset(&mut self) -> Option<(u32, u32)> {
    ...
    fn handle_reset_graphics(&mut self, width: u32, height: u32) {
        self.pending_output_reset = Some((width, height));
    ...
    fn handle_create_surface(&mut self, surface_id: u16, width: u16, height: u16, pixel_format: PixelFormat) {
        if let Some(decoder) = &mut self.h264_decoder {
            decoder.delete_surface(surface_id);
        }
        if width == 0 || height == 0 {
    ...
    fn decode_avc420(&mut self, surface_id: u16, dest_rect: &ExclusiveRectangle, bitmap_data: &[u8]) -> PduResult<()> {
        ...
        let frame = decoder
            .decode_surface(surface_id, stream.data)
            .map_err(|e| pdu_other_err!("H.264 decode", source: e))?;

        // AVC420 解码结果覆盖表面，regionRects 使用表面坐标。
        // 先完整验证所有掩码，再写入画面。
        for rect in &stream.rectangles {
            if rect.left > rect.right
                || rect.top > rect.bottom
                || rect.left < dest_rect.left
                || rect.top < dest_rect.top
                || rect.right > dest_rect.right
                || rect.bottom > dest_rect.bottom
                || u32::from(rect.right) > frame.width()
                || u32::from(rect.bottom) > frame.height()
            {
                return Err(pdu_other_err!("AVC420 region outside destination or decoded frame"));
            }
        }
        let stride = usize::try_from(frame.width()).map_err(|_| pdu_other_err!("AVC420 width"))? * 4;
        for rect in stream.rectangles {
            ...
            for row in rect.top..rect.bottom {
                let start = usize::from(row) * stride + usize::from(rect.left) * 4;
                let pixels = frame
                    .data()
                    .get(start..start + row_bytes)
                    .ok_or_else(|| pdu_other_err!("AVC420 truncated RGBA frame"))?;
