...
//! use Annex B start code prefixes per MS-RDPEGFX 2.2.4.4.
//!
...
/// EGFX client. The decoder receives Annex B NAL units from `RFX_AVC420_BITMAP_STREAM` payloads.
///
/// # Thread Safety
///
...
pub trait H264Decoder: Send {
    /// 按目标表面隔离预测帧上下文；默认适配只支持单流的解码器。
    fn decode_surface(&mut self, _surface_id: u16, data: &[u8]) -> DecoderResult<DecodedFrame> {
        self.decode(data)
    }
    ...
    /// Decode Annex B H.264 NAL units
    /// into an RGBA bitmap.
    ///
...
mod openh264_impl {
    ...
    ///
    /// This decoder reads Annex B NAL units, decodes to YUV420p, then converts
