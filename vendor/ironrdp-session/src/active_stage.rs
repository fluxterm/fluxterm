        ...
        let (mut stage_outputs, processor_updates) = match action {
            ...
            Action::X224 => {
                ...
                if let Some((width, height)) = reset {
                    let region = reset_graphics_image(image, width, height)?;
                    stage_outputs.push(ActiveStageOutput::GraphicsUpdate(region));
                }
                // 保留稀疏区域，面积合并由消费者决定。
                for update in graphics_updates {
                    if let Some(region) = composite_graphics_updates(image, [(update.region, update.data)])? {
                        stage_outputs.push(ActiveStageOutput::GraphicsUpdate(region));
                    }
                }
...
/// 在合成之前重建权威画面，尺寸相同的重置也清除旧像素。
fn reset_graphics_image(image: &mut DecodedImage, width: u32, height: u32) -> SessionResult<InclusiveRectangle> {
    let width = u16::try_from(width).map_err(|_| SessionError::general("invalid GFX width"))?;
    let height = u16::try_from(height).map_err(|_| SessionError::general("invalid GFX height"))?;
    if width == 0 || height == 0 || u64::from(width) * u64::from(height) > 16_777_216 {
        return Err(SessionError::general("GFX output exceeds pixel budget"));
    }
...
#[cfg(test)]
mod tests {
    /// 动态分辨率和同尺寸重置都必须清空旧画面，随后可合成新边界区域。
    #[test]
    fn gfx_reset_resizes_authoritative_image_before_composition() {
        let mut image = DecodedImage::new(PixelFormat::RgbA32, 2, 2);
        reset_graphics_image(&mut image, 4, 3).unwrap();
        ...
        assert!(
            composite_graphics_updates(&mut image, [(region, vec![10, 20, 30, 255])])
                .unwrap()
                .is_some()
        );
        assert_eq!(&image.data()[44..48], &[10, 20, 30, 255]);
        reset_graphics_image(&mut image, 4, 3).unwrap();
        assert!(image.data().iter().all(|&byte| byte == 0));
        assert!(reset_graphics_image(&mut image, 0, 3).is_err());
        assert!(reset_graphics_image(&mut image, 65535, 65535).is_err());
    }
