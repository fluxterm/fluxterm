...
export function getConfigDirectoryStatus() {
  return invokeTauriCommand<ConfigDirectoryStatus>("config_directory_status");
}

/** 保存用户选择的配置目录位置。 */
export function selectConfigDirectoryParent(parentDir: string) {
  return invokeTauriCommand<ConfigDirectoryStatus>(
...
export function resetConfigDirectory() {
  return invokeTauriCommand<ConfigDirectoryStatus>("config_directory_reset");
}

/** 保存正常配置不可用时仍需使用的应用语言。 */
export function setBootstrapLocale(locale: Locale) {
  return invokeTauriCommand<ConfigDirectoryStatus>("bootstrap_locale_set", {
    locale,
  });
}
