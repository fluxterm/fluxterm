...
本规范覆盖 React Webview、Tauri 命令层、Engine 和 OpenAI 客户端。用户可见的 AppEvent 活动记录仍是独立 UI 模型，不属于日志基础设施。

## 2. 日志所有权

1. 最终结果由掌握真实结果、错误和耗时的最低层记录。
2. 上层只传播错误或同步状态时不得重复记录同一结果。
3. 前端只记录纯 UI 故障；后端拥有的 SSH、SFTP、Proxy 和 AI 生命周期不在前端重复输出。
